...
##################### Here starts the exercise #####################
	...
	def objective(trial):
		...
		N_ESTIMATORS = int(trial.suggest_discrete_uniform('n_estimators', 10, 100, 10))
		MAX_DEPTH = int(trial.suggest_discrete_uniform('max_depth', 5, 20, 5))
		#params = ParameterGrid({'n_estimators': N_ESTIMATORS, 'max_depth': MAX_DEPTH})
		kf = KFold(n_splits = 5)
		...
		for train_index, val_index in kf.split(X_train):
			...
			#val_acc = cross_val_score(rfc, X, y, cv=10, scoring=’roc_auc’)
